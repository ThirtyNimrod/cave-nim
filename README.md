...
1. **Generate** (`python main.py generate`, runs daily via GitHub Actions cron
   at 14:00 UTC): asks Gemini for a 3-line proverb plus a matching icon from
   the curated icon library, renders the card with Playwright, and commits
   the PNG and a new `database/canon.json` entry to `main`.
2. **Review and post**: check the newly committed image in
   `assets/rendered/`, then post it to Instagram yourself.
...
see `database/canon.json` and `assets/rendered/`. Post `nim. 1.31` onward will
be generated by Gemini once you add a `GEMINI_API_KEY` (see below).
...
Set this locally in `.env` (gitignored) and as a **GitHub Actions repo
secret** (Settings → Secrets and variables → Actions) for CI.
...
for Gemini, the full architecture, current implementation status, the
original brand idea, planned future enhancements, and an AI-use disclosure.
